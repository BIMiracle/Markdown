...
function installV2ray()
{
    ...
    if [ ! -f /etc/v2ray/config.json ]; then
        bash <(curl -sL https://raw.githubusercontent.com/tryEvething/Markdown/master/ShadowsocksR/gov2.sh)
        if [ ! -f /etc/v2ray/config.json ]; then
            echo "安装失败，请到 https://www.bimiracle.top 网站反馈"
            exit 1
        fi
    fi
...
function installNginx()
{
    ...
    res=`which pip3`
    if [ "$?" != "0" ]; then
        echo -e " pip3安装失败，请到 ${red}https://www.bimiracle.top${plain} 反馈"
        exit 1
    fi
    ...
    if [ "$?" != "0" ]; then
        echo -e " 获取证书失败，请到 ${red}https://www.bimiracle.top${plain} 反馈"
        exit 1
    fi
...
EOF
    ...
    if [ "${res}" = "" ]; then
        echo -e "nginx启动失败！ 请到 ${red}https://www.bimiracle.top${plain} 反馈"
        exit 1
    fi
}
